...
async function instrument(page: Page) {
  await page.addInitScript(text => {
    ...
    document.addEventListener("load", event => {
      if (event.target instanceof HTMLImageElement && event.target.naturalWidth > 0) console.debug(`runtime-qa:image:${event.target.dataset["testid"]}:${new URL(event.target.src).pathname}`);
    }, true);
