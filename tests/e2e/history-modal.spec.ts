...
async function settleCurrentFonts(page:Page){
  await page.evaluate(async()=>{
    const origin=performance.timeOrigin;
    const drain=async()=>{
      let p=document.fonts.ready;
      await p;
      while(performance.timeOrigin===origin&&(document.fonts.status==="loading"||document.fonts.ready!==p)){
        p=document.fonts.ready;
        await p;
      }
    };
    await drain();
    await new Promise<void>(resolve=>queueMicrotask(resolve));
    if(performance.timeOrigin===origin&&document.fonts.status==="loading")await drain();
  });
}

async function capture(page:Page,info:TestInfo,name:string){
  await settleCurrentFonts(page);
...
test("long dialogue history stays in a keyboard-readable modal and restores play position",async({page},info)=>{
  ...
  await capture(page,info,"history-desktop.png");
  ...
  await page.setViewportSize({width:390,height:600});await capture(page,info,"history-mobile.png");
...
test("chapter labels and selected choices survive a manual save and reload",async({page},info)=>{
  ...
  await page.setViewportSize({width:390,height:700});await capture(page,info,"history-chapters-restored.png");
...
test("loading after a manuscript update identifies the snapshot and retains its original choice and history",async({page},info)=>{
  ...
  await page.setViewportSize({width:390,height:844});await expect(page.getByTestId("slot-row-0").locator(".slot-version-note")).toBeInViewport();await capture(page,info,"snapshot-version-mobile.png");
