...
test("a waiting tab never autosaves and opens the latest manuscript after the writer closes",async({page,context},info)=>{
  ...
  await expect.poll(()=>waiting.evaluate(k=>JSON.parse(localStorage.getItem(k)!).title,key)).toBe("동시 편집 · 최신 저장");
  ...
  await expect(waiting.getByRole("heading",{name:"다른 탭에서 편집 중입니다"})).toBeVisible();
  await expect.poll(()=>page.evaluate(k=>JSON.parse(localStorage.getItem(k)!).title,key)).toBe("동시 편집 · 최신 저장");
  await page.close();
  await expect(waiting.getByLabel("작품 제목")).toHaveValue("동시 편집 · 최신 저장");
  await waiting.getByLabel("작품 제목").fill("권한을 받은 탭의 편집");
  await expect.poll(()=>waiting.evaluate(k=>JSON.parse(localStorage.getItem(k)!).title,key)).toBe("권한을 받은 탭의 편집");
...
test("simultaneous opens yield exactly one editor and one waiting document",async({context})=>{
  ...
  await expect.poll(async()=>await first.getByLabel("작품 제목").count()+await second.getByLabel("작품 제목").count()).toBe(1);
