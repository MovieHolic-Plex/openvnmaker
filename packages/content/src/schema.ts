...
/** 장면을 다 보고 나갈 때 평가하는 조건부 출구. when 이 없으면 무조건 이동한다. */
export interface SceneRoute {
  readonly next: string;
  /** 이 조건을 만족할 때만 이 경로로 간다. 없으면 항상 적용된다. */
  readonly when?: LineCondition;
}
...
export interface Scene {
  ...
  /** 장면 진입 시점에 세팅할 플래그들. 첫 대사의 when 보다 먼저 적용된다. */
  readonly set?: Record<string, string | number | boolean>;
  ...
  /** 조건부 출구. 앞에서부터 평가해 처음으로 조건이 맞는 경로로 간다. 전부 실패하면 ending/next 로 폴백한다. */
