...
const out=['testsuite global:','    setup:','        $ _test.screenshot_directory = "tests/screenshots/credits-" + str(__import__("time").time_ns())','    teardown:','        exit','','testcase player_credits:','    $ preferences.text_cps = 0','    click "버전정보"','    assert screen "about"',`    assert ${renpyText(asset.provenance.credit)}`,'    pause .6','    screenshot "native-credits-title.png"','    click "돌아가기"','    assert screen "main_menu"','    run Start()',`    assert ${renpyText(first.lines[0]!.text)}`,'    pause .6','    screenshot "native-credits-toolbar.png"','    click "크레딧"','    assert screen "about"',`    assert ${renpyText(asset.provenance.credit)}`,'    click "돌아가기"',`    assert ${renpyText(first.lines[0]!.text)}`];
let scene=first;
...
out.push('    advance until screen "vn_ending"','    click "크레딧"','    assert screen "about"',`    assert ${renpyText(asset.provenance.credit)}`,'    click "돌아가기"','    assert screen "vn_ending"','    pause .6','    screenshot "native-credits-ending.png"','    click "타이틀로"','    assert screen "main_menu"','');
out.push('testcase literal_credits:','    $ vn_qa_original_media = vn_media_credits','    $ vn_media_credits = [{"name": "Literal credit", "provenance": {"credit": "[missing_variable] {b}literal{/b}", "creator": "Tester"}}]','    click "버전정보"','    assert screen "about"','    assert "[missing_variable] {b}literal{/b}"','    pause .6','    screenshot "native-credits-literal.png"','    $ vn_media_credits = vn_qa_original_media','');
if(script.credits?.some(credit=>credit.names.trim())){
  ...
  out.push('    pause .6','    screenshot "native-team-credits-authored.png"','');
}
