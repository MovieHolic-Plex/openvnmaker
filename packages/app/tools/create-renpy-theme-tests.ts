...
"testcase keyboard_choice:","    $ _test.timeout = 30","    $ _test.transition_timeout = .01","    $ preferences.text_cps = 0","    run Start()",'    advance until screen "choice"',"    pause .3",'    screenshot "choice-panel.png"',
// Explicit coordinates keep the harness from picking a random pointer position before a key.
'    keysym "K_DOWN" pos (0, 0)','    keysym "K_DOWN" pos (0, 0)',"    pause .2",'    screenshot "keyboard-choice.png"','    keysym "K_RETURN" pos (0, 0)','    assert not screen "choice"',`    assert eval vn_flags == json.loads(${JSON.stringify(JSON.stringify(secondFlags))})`,"    pause .3",'    screenshot "keyboard-continued.png"',"",
...
lines.push('    advance until screen "vn_ending"',"    pause .3",`    assert eval renpy.get_widget("vn_ending", "ending_title").get_all_text() == ${JSON.stringify(scene.ending)}`,'    screenshot "ending-panel.png"','    click "대사록"','    assert screen "history"','    click "돌아가기"','    assert screen "vn_ending"','    click "타이틀로"','    assert screen "main_menu"',"    pause .3",'    screenshot "returned-title.png"',"",
...
'    "선택 완료"',"    return","",
'testcase long_choices:',"    $ _test.timeout = 30",'    run Start("vn_theme_fixture")', '    assert screen "choice"',"    pause .3",'    screenshot "long-choices-top.png"','    keysym "K_PAGEDOWN" pos (0, 0)','    keysym "K_PAGEDOWN" pos (0, 0)',"    pause .3",'    screenshot "long-choices-bottom.png"','    click "선택지 8:"','    assert "선택 완료"','    assert eval vn_theme_result == 7',"");
