...
import { resolveInline, resolveText } from "./engine/inlineText.js";
...
export function StudioApp({recoveryInitial}:{recoveryInitial?:ReturnType<typeof loadProject>}={}) {
  ...
  const [hostCaps, setHostCaps] = useState<{ gateway: boolean; graph?: boolean } | null>(null);
      ...
      <main className="studio-center">
        ...
        {view === "stage" && <>
          <section className="preview-workspace"><div className="preview-meta"><span><i /> SCENE {String(sceneNumber).padStart(2, "0")}</span><div><Icon name="image" size={12} />{scene.backgroundUrl ? "프로젝트 원화" : scene.background}<span className="meta-divider" />{scene.bgm && <><Icon name="music" size={12} />{script.audioAssets?.find(asset=>asset.url===scene.bgm)?.name??(scene.bgm.startsWith("/assets/user/")?"사용자 음원":scene.bgm)}</>}</div></div>
            <section className="stage-fit" ref={stageRef} aria-label="16:9 게임 미리보기"><div className="preview-frame" style={{ width: `${previewWidth}px` }}><section className="stage" data-testid="studio-stage"><Stage background={scene.background} backgroundUrl={backgroundAt(scene,index,previewFlags)} cgUrl={cgAt(script,scene,index,previewFlags)} hideSprites={scene.hideSprites} framing={framingAt(scene,index,previewFlags)} characters={script.characters} sprites={spritesAt(scene,index,previewFlags)} speaking={line.speaker} chapter={null} sceneEpoch={0} transition="none" />{!artOnly && <DialogueBox speaker={(() => { const name = speakerName(script, line.speaker); return name === null ? null : resolveText(name, previewFlags); })()} color={speakerColor(script, line.speaker)} parts={resolveInline(line.text, previewFlags).parts} textLength={resolveInline(line.text, previewFlags).plain.length} typing={true} />}</section></div></section>
