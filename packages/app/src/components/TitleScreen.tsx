...
export function TitleScreen({ script, standalone=false, hasSave, onStart, onContinue, onLoad, onCredits }: Props) {
  ...
  return <section className="title-screen rain-title" data-testid="title-screen">
    <img className="title-bg" data-testid="bg-image" src={resolveRuntimeAsset(cover)} alt="" />
    <div className="rain-title-wash" />
    <div className="rain-title-header"><span>VNMAKER <b>{standalone ? "VISUAL NOVEL" : "ORIGINAL"}</b></span>{!standalone && <a href="/studio.html" data-testid="studio-button">작품 편집 ↗</a>}</div>
