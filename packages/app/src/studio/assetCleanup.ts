 ...
 */
export async function removeUnreferencedAssets(candidates:readonly string[],current:{id:string;script:VnScript}):Promise<{removed:string[];kept:string[]}>{
  ...
  const versions=await listAllVersionScripts().catch(()=>[] as VnScript[]);
  const scripts=[current.script,...listing.projects.filter(row=>row.id!==current.id).map(row=>row.script),...versions];
