...
export function ProjectRecovery({id,onRestore}:{id:string;onRestore:(script:VnScript)=>Promise<void>}){
  ...
  function downloadRaw(key:string,name:string){
    try{const raw=localStorage.getItem(key);if(raw===null){setStatus("내려받을 원문이 없습니다.");return;}const url=URL.createObjectURL(new Blob([raw],{type:"text/plain;charset=utf-8"}));const a=document.createElement("a");a.href=url;a.download=name;a.click();window.setTimeout(()=>URL.revokeObjectURL(url),5000);}catch{setStatus("저장 데이터 원문을 읽지 못했습니다.");}
  }
  async function restore(){
    if(!candidate)return;
    try{await onRestore(candidate.script);}catch{setStatus("손상 원문의 보존에 실패해 복구를 중단했습니다. 원문과 외부 백업을 내려받아 보관하세요.");}
  }
  return <section className="project-recovery" aria-label="원고 복구"><h2>작품 보관함에서 복구</h2><p role="status">{status}</p>{candidate&&<p><strong>{candidate.script.title}</strong><br/>{candidate.script.scenes.length}개 장면 · 보관함 저장 {new Date(candidate.updatedAt).toLocaleString()}<br/>이 시점 이후 저장하지 못한 편집은 포함되지 않을 수 있습니다.</p>}<div><button className="studio-button" type="button" onClick={()=>downloadRaw(PROJECT_KEY,"vnmaker-damaged-recovery.txt")}>손상 데이터 원문 받기</button><button className="studio-button" type="button" onClick={()=>downloadRaw(PRODUCTION_KEY,"vnmaker-legacy-checkpoint.json")}>이전 제작 체크포인트 원문 받기</button>{candidate&&<button className="studio-button primary" type="button" disabled={busy} onClick={restore}>이 보관함 원고로 복구</button>}</div></section>;
}
