...
export function AudioLibrary({script,onChange}:{script:VnScript;onChange:(script:VnScript)=>void}){
  const [kind,setKind]=useState<AudioAsset["kind"]>("bgm"),[busy,setBusy]=useState(false),[error,setError]=useState("");
  ...
  return <details className="line-direction audio-library"><summary>내 음원 보관함 · {script.audioAssets?.length??0}</summary><label className="field-help">작품 음악 페이드 (초)<input aria-label="작품 음악 페이드 (초)" type="number" min="0" max="10" step="0.1" value={script.musicFadeSeconds??1.2} onChange={event=>{const value=event.target.valueAsNumber;if(Number.isFinite(value)&&value>=0&&value<=10)onChange({...script,musicFadeSeconds:value});}}/></label><p className="field-help">음악 시작·교체·정지에 적용합니다. 0초는 즉시 전환, 기본은 1.2초입니다. 웹과 네이티브 출력에 전달됩니다.</p><p className="field-help">MP3·Ogg·16비트 PCM WAV를 원본 그대로 보관합니다. 파일당 50MB·30분, 한 번에 합계 200MB까지 지원합니다.</p>
