...
export function ArtImportButton({script,onImport}:{script:VnScript;onImport:(assets:Artwork[])=>void}){
  ...
  async function importFiles(files:FileList|null){
    ...
    try{
      ...
      onImport(rows.map(row=>({id:`user-${crypto.randomUUID()}`,name:row.originalName.replace(/\.[^.]+$/,""),kind,url:row.path,createdAt:new Date(row.createdAt).toISOString(),...(kind==="character"?{characterId:character,...(expression?{expression}:{})}:{})})));
  ...
  return <section className="art-import"><div className="art-section-title"><Icon name="upload"/><h2>내 원화 가져오기</h2></div>
    ...
    <p>PNG·JPEG·WebP를 원본 그대로 이 브라우저에 보관합니다. 투명 PNG도 지원합니다. 게임 ZIP에는 실제 파일이 함께 들어갑니다.</p>{error&&<p role="alert" className="art-error">{error}</p>}
  </section>;
}
