...
export function editIssue(current: VnScript, next: VnScript): string | null {
  try {
    ...
    for (const scene of changed) {
      ...
      // 의상 큐·cg 에셋 바인딩은 씬만 봐서는 검증할 수 없다(배우 선언·에셋 존재 여부) — 있으면 전체를 검사한다.
      ...
      const cgChanged = scene.cg !== previous.get(scene.id)?.cg;
      if (hasOutfit || cgChanged) { parseScript(next); return null; }
      parseScene(scene);
    }
    return null;
