...
export function AssetLibrary({ script, scene, projectEpoch, onChange, onPatchScene, generationEnabled = true, active = true }: Props) {
  ...
  return <section className="art-library" data-testid="art-library">
    ...
    <aside className="art-workbench">
      ...
      <StorePanel active={active} script={script} projectEpoch={projectEpoch} onChange={onChange} onApplyToScene={apply} onInstalled={artworks => { const first = artworks[0]; if (first) { setSelectedId(first.id); setFilter(first.kind); setSearch(""); } }} />
